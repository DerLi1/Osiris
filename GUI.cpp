...
void GUI::renderAimbotWindow() noexcept
{
    if (window.aimbot) {
		...
		ImGui::Checkbox("Draw FOV", &config.aimbot[currentWeapon].AimbotFov);
        ...
        ImGui::SliderFloat("Fov", &config.aimbot[currentWeapon].fov, 0.0f, 180.0f, "%.2f", 1.8f);
        ...
        ImGui::Checkbox("Kill-shot", &config.aimbot[currentWeapon].killshot);
...
void GUI::renderAntiAimWindow() noexcept
{
    if (window.antiAim) {
        if (!config.style.menuStyle) {
            ...
            ImGui::Begin("LegitAA", &window.antiAim, windowFlags);
        }
...
void GUI::renderTriggerbotWindow() noexcept
{
    if (window.triggerbot) {
        ...
        ImGui::Checkbox("Kill-shot", &config.triggerbot[currentWeapon].killshot);
...
void GUI::renderVisualsWindow() noexcept
{
    if (window.visuals) {
        ...
        ImGui::PopID();
        ImGui::PushID(5);
        ImGui::SliderFloat("", &config.visuals.brightness, 0.0f, 1.0f, "NightMode: %.2f");
        ImGui::PopID();
        ImGui::PopItemWidth();
        ImGui::Combo("Skybox", &config.visuals.skybox, "Default\0cs_baggage\0cs_tibet\0embassy\0italy\0jungle\0nukeblank\0office\0sky_daylight01\0sky_daylight02\0sky_daylight03\0sky_daylight04\0skycloudy01\0sky_night_flat\0sky_night02\0sky_day02_05\0sky_day02_05\0sky_dust\0sky_rural\0sky_venice\0vertigo_hdr\0vertigo\0vertigoblue_hdr\0vietnam\0");
...
void GUI::renderGuiStyle2() noexcept
{
    ...
    ImGui::Begin("Inverse", nullptr, windowFlags | ImGuiWindowFlags_NoTitleBar);
